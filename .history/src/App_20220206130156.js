...
function App() {
  ...
  const handleClick=(id)=>{
    ...
    if(flipped.length===0){  //még nincs bent a flipped tömbben
  ...
  function App() {
    ...
    const handleClick=(id)=>{
      setDisabled(true)
      if(flipped.length===0){ //vizsgálom a flipped tömb tartalmazza e az idt, amin a kattintás történt
        setFlipped([parseInt(id)])
        setDisabled(false)
      }else{
        if(flipped.includes(id)) //ha nem tartalmazza, beteszi a flippedbe
            return
