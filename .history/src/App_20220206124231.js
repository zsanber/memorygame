    ...
    if(flipped.length===0){  //azaz ha még nincs bent a flipped tömbben
      ...
      if(flipped.include(id))  //vizsgálom a flipped tömb tartalmazza e az idt, amin a kattintás történt
        ...
        if(isMatch(id)){
          ...
          setTimeout(resetCards,1000) //trueval visszatérítem, ha talál, beteszem a solvedba, ha nem talál, akkor visszaforgatjuk a kártyát
     }
  }

  const isMatch=(id)=> {
    const clickedCards=cards.find(obj=>obj.id==id)
    const flippedCards=cards.find(obj=>obj.id==flipped[0])
  }
